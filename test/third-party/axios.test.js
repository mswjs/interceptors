...
it('preserves the "auth" options', async () => {
    const getRequestPromise = new DeferredPromise();
    ...
    expect(request.headers.get('Authorization')).toBe(`Basic ${btoa('foo@bar.com:secret123')}`);
});
it('follows a mocked redirect response (xhr)', async () => {
    interceptor.on('request', ({ request, controller }) => {
        if (request.url.endsWith('/original')) {
            return controller.respondWith(Response.redirect('http://localhost:3000/redirected', 307));
        }
...
it('follows a mocked redirect response (http)', async () => {
    interceptor.on('request', ({ request, controller }) => {
        if (request.url.endsWith('/original')) {
            return controller.respondWith(Response.redirect('http://localhost/redirected', 307));
        }
        if (request.url.endsWith('/redirected')) {
            controller.respondWith(new Response('redirected response'));
        }

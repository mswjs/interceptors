...
function decodeMessage(encodedEngineIoPacket) {
    const decodedEngineIoPacket = decodePacket(encodedEngineIoPacket);
    if (decodedEngineIoPacket.type !== 'message') {
        return;
    }
    const decodedSocketIoPacket = decoder['decodeString'](decodedEngineIoPacket.data);
...
it('intercepts and modifies data sent to a socket.io server', async () => {
    ...
    const echoedMessagePromise = new DeferredPromise();

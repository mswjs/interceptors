...
it('throws if closing the unconnected server', async () => {
    const serverPromise = new DeferredPromise();
    interceptor.once('connection', ({ server }) => {
        serverPromise.resolve(server);
    });
    new WebSocket('wss://example.com');
    const server = await serverPromise;
    expect(() => server.close()).toThrow(`Failed to close server connection for "wss://example.com/": the connection is not open. Did you forget to call "server.connect()"?`);
});
...
it('forwards "close" events from the original server', async () => {
    ...
    const ws = new WebSocket(testServer.ws.url('/?close=1003,Cannot process payload'));
    ws.onclose = (event) => clientCloseListener(event.code, event.reason);
    // Must forward the original close event to the interceptor.
    await vi.waitFor(() => {
        expect(interceptorServerCloseListener).toHaveBeenCalledWith(1003, 'Cannot process payload');
    });
    // Must forward the original close to the intercepted client.
    await vi.waitFor(() => {
        expect(clientCloseListener).toHaveBeenCalledWith(1003, 'Cannot process payload');
    });
});

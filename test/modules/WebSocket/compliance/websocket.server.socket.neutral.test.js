...
it('throws an error when accessing "server.socket" before calling "server.connect()"', async () => {
    ...
    await expect(socketPromise).rejects.toThrow('Cannot access "socket" on the original WebSocket server object: the connection is not open. Did you forget to call `server.connect()`?');

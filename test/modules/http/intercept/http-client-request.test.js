...
it('restores the original ClientRequest class after disposal', async () => {
    interceptor.dispose();
    expect(http.ClientRequest, 'Failed to restore the ClientRequest class to its original implementation').toEqual(await import('node:http').then((exports) => exports.ClientRequest));
    const request = new http.ClientRequest('http://localhost/does-not-matter').on('error', () => { });

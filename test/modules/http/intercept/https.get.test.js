...
it('intercepts a GET request', async () => {
    ...
    const [{ request: requestFromListener, requestId, controller }] = resolver.mock.calls[0];
    expect(requestFromListener.method).toBe('GET');
    expect(requestFromListener.url).toBe(url);
    expect(Object.fromEntries(requestFromListener.headers.entries())).toMatchObject({
        'x-custom-header': 'yes',
    });
...
it('intercepts an https.get request given RequestOptions without a protocol', async () => {
    ...
    expect(request.headers.get('host')).toBe(`${httpServer.https.address.host}:${httpServer.https.address.port}`);

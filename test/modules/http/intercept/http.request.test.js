...
const httpServer = new HttpServer((app) => {
    const handleUserRequest = (_req, res) => {
...
it('intercepts an http.request path in url and options', async () => {
    ...
    const req = http.request(new URL(httpServer.http.url('/one')), { path: '/two' }, callback);
    req.end();
    await toWebResponse(req);
    expect(resolver).toHaveBeenCalledTimes(1);
    const [{ request, requestId, controller }] = resolver.mock.calls[0];
...
it('intercepts an http.request with a URL with "username" and "password"', async () => {
    ...
    new URL(`http://${username}:${password}@${httpServer.http.address.host}:${httpServer.http.address.port}/`));
    req.end();
    await toWebResponse(req);
    expect(resolver).toHaveBeenCalledTimes(1);
    const [{ request }] = resolver.mock.calls[0];
    expect(request.method).toBe('GET');
    expect(request.url).toBe(httpServer.http.url('/'));
    expect(request.headers.get('authorization')).toBe(`Basic ${btoa(`${username}:${password}`)}`);
    expect(request.credentials).toBe('same-origin');
    expect(request.body).toBe(null);
});

...
it('preserves raw response headers (object init)', async () => {
    ...
    expect(rawResponse.rawHeaders).toEqual(expect.arrayContaining(['X-CustoM-HeadeR', 'Yes']));
...
it('preserves raw response headers (set after init)', async () => {
    ...
    const request = http.get(httpServer.http.url('/'));
    const [response, rawResponse] = await toWebResponse(request);
    expect(rawResponse.rawHeaders).toEqual(expect.arrayContaining(['X-CustoM-HeadeR', 'Yes', 'x-My-Header', '1']));
    expect(Object.fromEntries(response.headers)).toStrictEqual({
        'x-custom-header': 'Yes',
        'x-my-header': '1',
    });
...
it('preserves raw response headers (append after init)', async () => {
    ...
    const request = http.get(httpServer.http.url('/'));
    const [response, rawResponse] = await toWebResponse(request);
    expect(rawResponse.rawHeaders).toEqual(expect.arrayContaining([
        'X-CustoM-HeadeR',
        'Yes',
        'x-my-header',
        '1',

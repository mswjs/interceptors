...
it('allows reusing the same socket for multiple bypassed requests', async () => {
    ...
    });
    {
        ...
        expect.soft(response.status).toBe(200);
        await expect.soft(response.text()).resolves.toBe('original');
        expect(requestListener).toHaveBeenCalledExactlyOnceWith(expect.objectContaining({
            method: 'GET',
    ...
    }
    {
        ...
        expect(requestListener).toHaveBeenNthCalledWith(2, expect.objectContaining({
            method: 'GET',

...
it('calls the write callback when writing an empty string', async () => {
    const request = http.request(httpServer.http.url('/resource/write-empty-cb'), {
        method: 'POST',
    });
...
it('calls the write callback when writing an empty Buffer', async () => {
    const request = http.request(httpServer.http.url('/resource/write-callback'), {
        method: 'POST',
    });
    const writeCallback = vi.fn();
...
it('supports ending a mocked request in a write callback', async () => {
    ...
    const request = http.request(httpServer.http.url('/resource/mocked-end-after-write'), {
        method: 'POST',
    });
...
it('supports ending a bypassed request in a write callback', async () => {
    const request = http.request(httpServer.http.url('/resource/real-end-after-write'), {
        method: 'POST',

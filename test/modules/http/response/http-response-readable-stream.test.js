...
it('handles delayed mock response stream errors as response errors', async () => {
    ...
    await expect
        ...
        .toHaveBeenCalledExactlyOnceWith(expect.objectContaining({ code: 'ECONNRESET' }));
    expect
        .soft(responseDataListener)
        .toHaveBeenCalledExactlyOnceWith(Buffer.from('first-chunk'));
    expect.soft(request.destroyed).toBe(true);
    expect.soft(requestResponseListener).toHaveBeenCalledExactlyOnceWith(expect.objectContaining({
        statusCode: 200,
        statusMessage: 'OK',
    }));
...
it('treats unhandled exceptions during bypass response stream as response errors', async () => {
    ...
    expect
        .soft(responseDataListener)
        .toHaveBeenCalledExactlyOnceWith(Buffer.from('first-chunk'));
    expect.soft(responseErrorListener).toHaveBeenCalledExactlyOnceWith(expect.objectContaining({
        code: 'ECONNRESET',
        message: 'aborted',
    }));
});

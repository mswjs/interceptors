 ...
 */
it.skipIf(!IS_BROWSER)('returns an empty string for a mocked response to an "about:" request', async () => {
...
it.skipIf(!IS_BROWSER)('returns an empty string for a cloned mocked response to an "about:" request', async () => {
    interceptor.on('request', ({ controller }) => {
        controller.respondWith(new Response('hello world'));
    });
    const response = await fetch('about:');

...
it('aborts unsent request when the original request is aborted', async ({ task, }) => {
    const controller = new AbortController();
    const abortErrorPromise = fetch(server.http.url('/resource'), {
        signal: controller.signal,
    }).then(() => expect.fail('must not return any response'), (error) => error);
    controller.abort();
    const abortError = await abortErrorPromise;
    expect.soft(abortError.name).toBe('AbortError');
    expect
        .soft(abortError.message)
        .toBe(task.file.projectName === 'browser'
        ? 'signal is aborted without reason'
        : 'This operation was aborted');
});
it('aborts a pending request when the original request is aborted', async ({ task, }) => {
    const requestListenerCalled = new DeferredPromise();
    const fetchRejected = new DeferredPromise();
    const lateRespondWithResult = new DeferredPromise();
    ...
    expect(lateRespondWithError?.message).toBe(`Failed to respond to the "GET ${server.http.url('/delay')}" request with "200 OK": the request has already been handled (3)`);
});
...
it('respects requests aborted before they are dispatched', async ({ task, }) => {
    ...
    controller.abort();
    const abortError = await fetch(request).then(() => expect.fail('must not return any response'), (error) => error);
    expect.soft(abortError.name).toBe('AbortError');
    expect
        .soft(abortError.message)
        .toBe(task.file.projectName === 'browser'
...
it('aborts the pending request via "AbortSignal.timeout"', async ({ task, }) => {
    const fetchRejected = new DeferredPromise();
    const requestListenerDone = new DeferredPromise();

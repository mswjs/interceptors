...
it.skipIf(IS_BROWSER)('decompresses a mocked "content-encoding: br" response body', async () => {
    const { brotliCompressSync } = await import('node:zlib');
    interceptor.on('request', ({ controller }) => {
        controller.respondWith(new Response(brotliCompressSync('hello world'), {
...
it.skipIf(IS_BROWSER)('decompresses a bypassed "content-encoding: br" response body', async () => {
    const response = await fetch(server.http.url('/compressed'), {
...
it.skipIf(IS_BROWSER)('decompresses a mocked "content-encoding: gzip, br" response body', async () => {
    const { brotliCompressSync, gzipSync } = await import('node:zlib');
    interceptor.on('request', ({ controller }) => {
        controller.respondWith(new Response(brotliCompressSync(gzipSync('hello world')), {
...
it('exposes a compressed bypassed response in the "response" listener', async () => {
    const responseFromListenerPromise = Promise.withResolvers();
    ...
    const expectedBytes = await new Response(new Blob(['hello world'])
        .stream()
        .pipeThrough(new CompressionStream('gzip'))).bytes();
    ...
    if (IS_BROWSER) {
        await expect(responseFromListener.bytes()).resolves.toEqual(new TextEncoder().encode('hello world'));
    }
    else {
        await expect(responseFromListener.bytes()).resolves.toEqual(await new Response(new Blob(['hello world'])
            .stream()
            .pipeThrough(new CompressionStream('gzip'))).bytes());
    }
    await expect(response.text()).resolves.toBe('hello world');

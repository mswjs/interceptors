...
it('throws if "respondWith" is called multiple times within the same listener', async () => {
    ...
    expect(error).toHaveProperty('message', `Failed to respond to the "GET http://localhost/resource" request with "200 OK": the request has already been handled (2)`);
...
it('errors the request via "errorWith"', async ({ task }) => {
    ...
    const unhandledExceptionListener = vi.fn();
    interceptor.on('unhandledException', unhandledExceptionListener);
    const requestError = await fetch('http://localhost/resource').then(() => null, (error) => error);
...
it('throws if "errorWith" is called multiple times within the same listener', async ({ task, }) => {
    const errorPromise = new DeferredPromise();
    interceptor.on('request', ({ controller }) => {
    ...
    expect(error).toBeInstanceOf(Error);
    expect(error).toHaveProperty('name', 'InterceptorError');
    expect(error).toHaveProperty('message', `Failed to error the "GET http://localhost/resource" request with "Error: two": the request has already been handled (3)`);
...
it('throws if "errorWith" is called multiple times across different listeners', async ({ task, }) => {
    const errorPromise = new DeferredPromise();
    interceptor.on('request', ({ controller }) => {
        controller.errorWith(new Error('one'));
    });
...
it('throws if "respondWith" is called after "errorWith" was called', async ({ task, }) => {
    const errorPromise = new DeferredPromise();
    interceptor.on('request', ({ controller }) => {
        controller.errorWith(new Error('one'));
        try {
    ...
    expect(error).toBeInstanceOf(Error);
    expect(error).toHaveProperty('name', 'InterceptorError');
    expect(error).toHaveProperty('message', `Failed to respond to the "GET http://localhost/resource" request with "200 OK": the request has already been handled (3)`);
    // Must reject the response promise with the given error.
    expect(requestError).toBeInstanceOf(Error);
    if (task.file.projectName === 'browser') {
        expect(requestError).toHaveProperty('name', 'Error');
...
it('throws if "errorWith" is called after "respondWith" was called', async () => {
    ...
    expect(error).toBeInstanceOf(Error);
    expect(error).toHaveProperty('name', 'InterceptorError');
    expect(error).toHaveProperty('message', `Failed to error the "GET http://localhost/resource" request with "Error: one": the request has already been handled (2)`);

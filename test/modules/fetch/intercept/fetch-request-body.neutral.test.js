...
it('intercepts a request without a body', async () => {
    const pendingRequestBody = new DeferredPromise();
...
it('intercepts a request with a FormData body', async () => {
    ...
    expect(requestBody).toMatch(/content-disposition: form-data; name="username"\r\n\r\njohn\r\n/i);
    expect(requestBody).toMatch(/content-disposition: form-data; name="password"\r\n\r\nsecret-123\r\n/i);
});
...
it('intercepts a request with a URLSearchParams body', async () => {
    ...
    await expect(pendingRequestBody).resolves.toBe('username=john&password=secret-123');
});

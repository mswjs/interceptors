...
describe.each(['http', 'https'])('%s', (protocol) => {
    ...
    it('intercepts a POST request', async () => {
        ...
        expect(request.headers.get('x-custom-header')).toBe('yes');
        expect(request.credentials).toBe('same-origin');
        await expect(requestBodyPromise).resolves.toBe(JSON.stringify({ body: true }));
        expect(controller).toBeInstanceOf(RequestController);
        expect(requestId).toMatch(/^\w{9,}$/);
        expect(response.status).toBe(200);
...
it('sets "credentials" to "include" on the intercepted request', async ({ task, }) => {
    const requestCredentialsPromise = new DeferredPromise();
...
it('sets "credentials" to "omit" on the intercepted request', async ({ task, }) => {
    const requestCredentialsPromise = new DeferredPromise();
    interceptor.on('request', ({ request, controller }) => {
        requestCredentialsPromise.resolve(request.credentials);
        controller.respondWith(new Response());

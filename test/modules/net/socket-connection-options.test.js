...
it('exposes the known options for "net.connect(options)"', async () => {
    ...
    const lookupFunction = (hostname, options, callback) => {
    ...
    expect(connectionListener).toHaveBeenCalledWith(expect.objectContaining({
        port: 1337,
        host: '127.0.0.1',
        family: 4,
        hints: 0,
...
it('ignores unknown options', async () => {
    ...
    expect.soft(exposedConnectionOptions).toEqual(expect.objectContaining({
        port: 1337,
        host: '127.0.0.1',
    }));

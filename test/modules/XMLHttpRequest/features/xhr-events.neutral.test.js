...
it('emits events for a handled request', async ({ task }) => {
    ...
    {
        const [{ request, requestId }] = requestListener.mock.calls[hasPreflight ? 1 : 0];
        expect.soft(request).toBeInstanceOf(Request);
    ...
    // Mocked response.
    {
        const [{ response, request, requestId }] = responseListener.mock.calls[hasPreflight ? 1 : 0];
...
it('emits events for a bypassed request', async ({ task }) => {
    ...
    {
        const [{ request, controller, requestId }] = requestListener.mock.calls[hasPreflight ? 1 : 0];
        expect.soft(request).toBeInstanceOf(Request);
        expect.soft(request.method).toBe('GET');
        expect.soft(request.url).toBe(url.href);
        expect.soft(controller).toBeInstanceOf(RequestController);
    ...
    {
        const [responseParams] = responseListener.mock.calls[hasPreflight ? 1 : 0];

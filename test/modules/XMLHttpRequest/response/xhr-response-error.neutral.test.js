...
import { spyOnXMLHttpRequest, waitForXMLHttpRequest, } from '#/test/setup/helpers-neutral';
...
it('intercepts a bypassed request with a network error response', async ({ task, }) => {
...
it('treats Response.error() as a request error for an HTTP request', async ({ task, }) => {
...
it('treats Response.error() as a request error for an HTTPS request', async ({ task, }) => {
    interceptor.on('request', ({ controller }) => {
        controller.respondWith(Response.error());
    });
    const request = new XMLHttpRequest();

...
import { arrayBufferFrom, toArrayBuffer, waitForXMLHttpRequest, } from '#/test/setup/helpers-neutral';
...
it('reads the mocked ArrayBuffer response as-is if "responseType" is set to "arraybuffer"', async () => {
    interceptor.on('request', ({ controller }) => {
        controller.respondWith(new Response(new TextEncoder().encode('hello world'), {
...
it('reads the mocked json response as ArrayBuffer if "responseType" is set to "arraybuffer"', async () => {
    ...
    interceptor.on('request', ({ controller }) => {
        controller.respondWith(Response.json(json, {
            headers: {
                'access-control-allow-origin': '*',
            },
        }));
...
it('reads the mocked blob response as ArrayBuffer if "responseType" is set to "arraybuffer"', async () => {
    interceptor.on('request', ({ controller }) => {
        controller.respondWith(new Response(new Blob(['hello world'], { type: 'text/plain' }), {
            headers: { 'access-control-allow-origin': '*' },
        }));
    });
    const request = new XMLHttpRequest();

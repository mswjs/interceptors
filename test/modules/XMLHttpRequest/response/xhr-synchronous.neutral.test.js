 ...
 */
it('prints a warning upon attempts to handle a synchronous XMLHttpRequest', async ({ task, }) => {
    interceptor.on('request', ({ controller }) => {
        controller.respondWith(new Response('must not receive this', {
            headers: {
    ...
    if (task.file.projectName === 'browser') {
        await expect
            .poll(() => console.warn)
            .toHaveBeenCalledExactlyOnceWith(`Failed to intercept an XMLHttpRequest (GET ${server.http.url('/')}): synchronous requests are not supported. This request will be performed as-is.`);

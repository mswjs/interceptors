...
it('treats "controller.errorWith()" as a request error for an HTTP request', async ({ task, }) => {
...
it('treats "controller.errorWith()" as a request error for an HTTPS request', async ({ task, }) => {
    interceptor.on('request', ({ controller }) => {
        controller.errorWith(new Error('Network failure'));
    });
    const request = new XMLHttpRequest();

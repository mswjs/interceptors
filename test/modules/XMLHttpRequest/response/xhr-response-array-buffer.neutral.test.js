...
import { spyOnXMLHttpRequest, toArrayBuffer, waitForXMLHttpRequest, } from '#/test/setup/helpers-neutral';
...
it('intercepts a bypassed request with an ArrayBuffer response', async ({ task, }) => {
...
it('responds with a mocked ArrayBuffer response to an HTTP request', async ({ task, }) => {
    const buffer = new TextEncoder().encode('hello world');
...
it('responds with a mocked ArrayBuffer response to an HTTPS request', async ({ task, }) => {
    const buffer = new TextEncoder().encode('hello world');
    interceptor.on('request', ({ controller }) => {
        controller.respondWith(new Response(buffer, {
            headers: {

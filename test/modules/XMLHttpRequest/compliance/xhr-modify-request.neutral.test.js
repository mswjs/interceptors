...
it('allows modifying outgoing request headers', async () => {
    ...
    expect
        .soft(request.getResponseHeader('x-delete-header'), 'XMLHttpRequest headers cannot be deleted')
        .toBe('a');
    expect
        .soft(request.getResponseHeader('x-append-header'), 'Appends a new header value')
        .toBe('1, 2');
    expect

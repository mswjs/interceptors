...
])(`does not fail when unsetting event handlers for a successful %s response`, async (_, getUrl) => {
...
])(`does not fail when unsetting event handlers for a %s error response`, async (_, getUrl) => {
    interceptor.on('request', ({ request, controller }) => {
        if (request.method === 'OPTIONS') {
            return controller.respondWith(new Response(null, {
                status: 204,
        ...
        controller.respondWith(new Response('Internal Server Error', { status: 500 }));
    });
    const url = getUrl();
    const request = new XMLHttpRequest();
...
])(`does not fail when unsetting event handlers for a %s request error`, async (_, getUrl) => {
    interceptor.on('request', ({ request, controller }) => {
        if (request.method === 'OPTIONS') {
            return controller.respondWith(new Response(null, {
                status: 204,

...
import { arrayBufferFrom, waitForXMLHttpRequest, } from '#/test/setup/helpers-neutral';
const interceptor = new XMLHttpRequestInterceptor();
interceptor.on('request', ({ controller }) => {
    controller.respondWith(new Response(JSON.stringify({
        firstName: 'John',
        lastName: 'Maverick',
    }), {
        headers: {
...
it('responds with a Blob when "responseType" equals "blob"', async () => {
    ...
    await expect(responseBlob.text()).resolves.toBe(JSON.stringify({
        firstName: 'John',
        lastName: 'Maverick',
    }));
});
it('responds with an ArrayBuffer when "responseType" equals "arraybuffer"', async () => {
    ...
    request.send();
    await waitForXMLHttpRequest(request);
    const expectedArrayBuffer = arrayBufferFrom(JSON.stringify({
        firstName: 'John',
        lastName: 'Maverick',
    }));

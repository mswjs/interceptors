...
test('emits custom upload events when uploading text', async () => {
  ...
  await createXMLHttpRequest((req) => {
    ...
    req.upload.addEventListener('loadstart', (event: any) => {
      ...
      // Request body length is already known.
      expect(event.total).toEqual(12)
    })

    req.upload.addEventListener('progress', (event) => {
      record('progress')
      expect(event.lengthComputable).toEqual(true)
      expect(event.loaded).toEqual(12)
      expect(event.total).toEqual(12)
    })
...
test('calls custom upload callbacks when uploading text', async () => {
  const record = jest.fn((_name: string) => {})
